...
fun mayorTriangulo(listaTringulos:MutableList<Triple<String,Double,Double>>):String{
    var lista_area:MutableList<Double> = mutableListOf()
    var list_name:MutableList<String> = mutableListOf()
    for((nombre,base,altura) in listaTringulos){
        var area = areaTriangulo(base,altura)
        lista_area.add(area)
        list_name.add(nombre)
    }

    var areas =  lista_area.maxOrNull()
    var indice = lista_area.indexOf(areas)
    return list_name.get(indice)
}
