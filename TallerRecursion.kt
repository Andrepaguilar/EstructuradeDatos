...
fun serie(n: Int): Double {
    when {
        n == 1 -> return (1 / (2 * n.toDouble() + 1))
        else -> return (1 / (2 * n.toDouble() + 1)) + serie(n - 1)
...
fun digitoMasGrande(n: Int): Int {
    if (n in 0..9){
        ...
        return if(n%10 >= digitoMasGrande(n/10)){n%10} else{digitoMasGrande(n/10)}
    }
}
...
fun <T> imprimirLista(lista: IList<T>) {
    ...
    if(tamaño == 1){
        ...
        return println( imprimirLista(lista))
    }
}
